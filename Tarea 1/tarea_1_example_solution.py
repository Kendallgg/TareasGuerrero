...
# Se utilizarán para validar que los strings contengan solo letras ASCII
# (A–Z, a–z) y dígitos (0–9).
import re
...
err_cadena_invalida = -200    # 'cadena' contiene caracteres fuera de ASCII
err_caracter_invalido = -300
# 'caracter' no es string de 1 char alfanumérico ASCII.
...
def count_char(cadena, caracter):
    ...
    # b) Validar que 'cadena' contenga solo letras
    # y dígitos ASCII (sin espacios ni símbolos).
    ...
    if not (isinstance(caracter, str)
            and len(caracter) == 1 and _ascii_alnum_re.match(caracter)):
...
def multiplo_2(base, multiplo):
    ...
    # - El test permite base = 0 (por eso se usa base >= 0)
    # y exige multiplo > 0.
    ...
    # equivalente a multiplicar por 2**desplazamiento.
